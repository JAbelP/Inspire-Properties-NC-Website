...
function Myheader() {
  ...
  return (
    <div>
      <div className="flex flex-row  items-start pt-8 sm:pr-10 text-2xl text-black bg-white h-25">
        {/* ----------------LOGO-------------------------- */}
        <div>
          <Link href={"/"}>
        ...
        <div className="hidden lg:block mx-auto">
          <div className="flex-grow flex-shrink-0 sm:pl-6 text-4xl">
            <div className="flex flex-row  gap-x-9 flex-shrink-1 flex-wrap">
              <Link href={"/"}>
                <div>
                  <HeaderButton
                    label={"Home"}
                    className={"bg-green-700 my-2 rounded-lg"}
                  />
                </div>
              </Link>
              <div>
              ...
              <div>
                <Link href={"/BeforeAndAfter"}>
                  <HeaderButton label={"Before And After  "} />
                </Link>
              </div>
              {/* <div>
                <Link href={"/Services"}>
      ...
      {/* ---------------------------- tray  ------------------------ */}
      <div
        className={`fixed w-2/3 h-full bg-greenLogo right-0 top-0 duration-300 z-20 transition-transform transform ${
      ...
      <div
        ...
        className={`fixed w-1/3 h-full bg-green-950 opacity-75 left-0 top-0 duration-300 z-20 transition-transform transform ${
          menuOpen ? "translate-x-0" : "-translate-x-full"
        } text-center text-5xl pt-36`}
      ></div>
      {/* ---------------------------- tray  ------------------------ */}
...
const HeaderButton = ({
  label,
  className = "bg-white hover:bg-greenLogo my-2 rounded-lg ",
}) => (
